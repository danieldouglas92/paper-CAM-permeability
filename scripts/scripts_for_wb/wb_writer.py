...
def mass_conserving_model(thermal_model, density, plate_vel, coupling_depth, ridge_coords, taper, max_slab_top, min_slab_top, first_or_last='both'):
    ...
    string += '{"model":"mass conserving", "reference model name": "' + thermal_model + '", "adiabatic heating":true, "density":' + str(density) + ', "spreading velocity":' + str(plate_vel) + ', "subducting velocity":' + str(plate_vel) + ', "coupling depth":' + str(coupling_depth) + \
...
def composition_feature_string(model_name, comp_index, max_depth, min_depth, is_subducting=False, is_fault=False, first_or_last='both', operation='replace'):
    ...
    is_subducting = True or False, whether the current feature is a subducting slab
    is_fault = True or False, whether the current feature is a fault
    first_or_last = whether this feature is the first or last temperature feature. 'first' is the first feature, 'last' is the last feature, 'both'
                    is if this is the only feature (i.e. the 'first' and the 'last')
    '''
    if first_or_last == 'first' or first_or_last == 'both':
    ...
    if is_subducting == False and is_fault == False:
        string += '{"model":"' + str(model_name) + '", "compositions":[' + str(comp_index) + '], "max depth":' + str(max_depth) + \
                  ', "min depth":' + str(min_depth) + ', "operation":"' + str(operation) + '"}'
        ...
        string += '{"model":"' + str(model_name) + '", "compositions":[' + str(comp_index) + '], "operation":"' + str(operation) + '", ' + \
                  '"max distance fault center":' + str(max_depth) + ', "min distance fault center":' + str(min_depth) + '}'
...
########################################## WATER CONTENT FEATURES FOR THE WB FILE ##########################################
############################# Functions which write strings segments for composition fields ##############################
##########################################################################################################################
def tian_water_content_string(comp_index, max_depth, min_depth, lithology, initial_water_content, cutoff_pressure, is_subducting=False, first_or_last='both', operation='replace', COH=False):

    '''
    Writes the string for the tian water content feature to the world builder file
    ...
    is_subducting = True or False, whether the current feature is a subducting slab
    first_or_last = whether this feature is the first or last temperature feature. 'first' is the first feature, 'last' is the last feature, 'both'
                    is if this is the only feature (i.e. the 'first' and the 'last'), or 'neither'
    operation     = whether the composition defined in this string should replace others defined before it, add on top of it, or subtract away from it
    '''
    water_content_model_string = '{"model": "tian water content", '
    if COH:
        water_content_model_string = '{"model": "coh water content", '
    ...
    if is_subducting == False:
        string += water_content_model_string + '"compositions":[' + str(comp_index) + '], "min depth":' + str(min_depth) + \
                  ', "max depth":' + str(max_depth) + ', "lithology": "' + lithology + '", "initial water content":' + str(initial_water_content) + \
                  ', "cutoff pressure":' + str(cutoff_pressure) + ', "operation": "' + str(operation) + '"}'

    elif is_subducting:
        string += water_content_model_string + '"compositions":[' + str(comp_index) + '], "min distance slab top":' + str(min_depth) + \
                  ', "max distance slab top":' + str(max_depth) + ', "lithology": "' + lithology + '", "initial water content":' + str(initial_water_content) + \
                  ', "cutoff pressure":' + str(cutoff_pressure) + ', "operation": "' + operation + '"}'
